...
class StyleDiscriminator(nn.Module):
    def __init__(self, in_channel, in_res, ch_mul=64, ch_max=512, auxilary=False, **kwargs):
        super().__init__()
        ...
        self.disc_out = DiscriminatorHead(in_channel=in_channels, disc_stddev=True, auxilary=auxilary)

    def forward(self, x):
        x = self.conv_in(x)
        x = self.layers(x)
        if self.auxilary:
            out, aux = self.disc_out(x)
            return out, aux
        else:
            out = self.disc_out(x)
            return out
...
class DiscriminatorHead(nn.Module):
    def __init__(self, in_channel, disc_stddev=False, auxilary=False):
        super().__init__()
        ...
        if self.auxilary:
            self.aux_layer = nn.Sequential(
                nn.Flatten(),
                EqualLinear(in_channel=in_channel * 4 * 4, out_channel=in_channel, activate=True),
                EqualLinear(in_channel=in_channel, out_channel=3),
        )
